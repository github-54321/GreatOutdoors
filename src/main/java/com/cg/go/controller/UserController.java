...
public class UserController {
	...
	public ResponseEntity<Userdata> loginUser(@PathVariable("userId") int userId, @PathVariable("userPassword") String userPassword) {
		LOGGER.info("Userdata loginUser() started");
		Userdata user = new Userdata();
		user.setUserId(userId);
	    user.setUserPassword(userPassword);
		Userdata login =  userService.loginUser(user);
		LOGGER.info("Userdata loginUser() completed");
		return new ResponseEntity<Userdata>(login,HttpStatus.OK);
	   }
	
	
	@GetMapping(path= "/viewuserbyid/{userId}")
	public ResponseEntity<Userdata> viewCustomerByCustomerId(@PathVariable Integer userId){
