...
public class CustomerServiceImpl implements ICustomerService {
	...
	@Override
	@Transactional
	public Customer loginCustomer(String customerEmail, String customerPassword) throws IDNotFoundException {
		LOGGER.info("Customer loginCustomer() started");
		
		 Optional<Customer> cust = customerRepository.findByEmail(customerEmail);
		 Customer customer1 = cust.get();
		 LOGGER.info("Userdata loginUser() ended");
		 if(!(customer1.getEmail().equalsIgnoreCase(customerEmail)) || !(customer1.getCustomerPassword().equalsIgnoreCase(customerPassword))) {		 
